...
AVANTIKA_PREAMBLE = """You are AVANTIKA AI — the most ADVANCED, PREMIUM, and INTELLIGENT AI Assistant ever created. 💎✨👑
...
async def setrules(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"📜 *Group Rules Set!* ✅\n\n{group_rules[cid]}", parse_mode="Markdown")
...
async def warn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if update.effective_user.id not in [a.user.id for a in await context.bot.get_chat_administrators(cid)]:
            await update.message.reply_text("❌ *Sirf Admin warn kar sakta hai!* 👑", parse_mode="Markdown"); return
    except: return
    if not update.message.reply_to_message: await update.message.reply_text("⚠️ _Kisi message pe reply karo!_"); return
    t = update.message.reply_to_message.from_user
    ...
    await update.message.reply_text(
        f"⚠️ *Warning!* 👤 {t.first_name}\n"
        f"📊 *{wc}/3* {'🔴 Mute recommended!' if wc>=3 else '⚡ Be careful!'}",
        parse_mode="Markdown"
    )
...
async def ban_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if update.effective_user.id not in [a.user.id for a in admins]:
            await update.message.reply_text("❌ *Sirf Group Admin ban kar sakta hai!* 👑", parse_mode="Markdown")
            return
    except:
        await update.message.reply_text("❌ _Admin check fail!_", parse_mode="Markdown")
        return
    ...
    # Manual ID se target lo
    elif context.args:
        try:
            target_id = int(context.args[0])
            member = await context.bot.get_chat_member(cid, target_id)
            target = member.user
        except:
            await update.message.reply_text("❌ *User ID galat ya user group mein nahi hai!*", parse_mode="Markdown")
            return
    else:
        await update.message.reply_text(
            "🔨 *BAN SYSTEM* 🔨\n\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n"
            "📌 *Reply karke:* Kisi message pe reply karo + `/ban`\n"
            "📌 *Manual:* `/ban 123456789`\n\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n"
            "🔓 *Unban:* `/unban user_id`",
            parse_mode="Markdown"
        )
    ...
    if not target:
        await update.message.reply_text("❌ _User nahi mila!_")
        return
    
    if target.id == update.effective_user.id:
        await update.message.reply_text("😅 _Khud ko ban nahi kar sakte!_")
        return
    
    if target.is_bot:
        await update.message.reply_text("🤖 _Bot ko ban nahi kar sakte!_")
        return
    
    try:
        await context.bot.ban_chat_member(chat_id=cid, user_id=target.id)
        ...
        await update.message.reply_text(
            ...
            f"🆔 *ID:* `{target.id}`\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"⚠️ _User ab group join nahi kar sakta!_\n"
            f"🔓 _Unban karne ke liye:_ `/unban {target.id}`",
            parse_mode="Markdown"
        )
...
async def unban_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("📝 *USAGE:* `/unban user_id`\n\n_User ID daalkar unban karo!_", parse_mode="Markdown")
        return
    ...
    try:
        ...
        await update.message.reply_text(
            ...
            f"🆔 *User ID:* `{user_id}`\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"🎉 _User ab wapas join kar sakta hai!_",
            parse_mode="Markdown"
        )
    except Exception as e:
        await update.message.reply_text(
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ct == ChatType.PRIVATE:
        if uid == OWNER_USER_ID:
            ...
            user_history[cid] = []
            await update.message.reply_text("✅ *Access Granted!*\n\n💬 _Kuch bhi puchho — AVANTIKA jawab degi!_ 💎", parse_mode="Markdown")
        else:
