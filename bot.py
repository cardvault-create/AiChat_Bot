...
async def setrules(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📜 *Group Rules Set!* ✅", parse_mode="Markdown")
...
async def warn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"⚠️ *Warning!* 👤 {t.first_name}\n📊 *{wc}/3* {'🔴 Mute recommended!' if wc>=3 else '⚡ Be careful!'}", parse_mode="Markdown")
...
async def clearwarns(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ *Warnings Cleared!*", parse_mode="Markdown")

async def ban_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    t = update.message.reply_to_message.from_user if update.message.reply_to_message else None
    if not t: return
    try: await context.bot.ban_chat_member(update.effective_chat.id, t.id); await update.message.reply_text(f"🔨 *BANNED!* 👤 {t.first_name}", parse_mode="Markdown")
    except: pass

async def unban_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args: return
    try: await context.bot.unban_chat_member(update.effective_chat.id, int(context.args[0])); await update.message.reply_text("✅ *UNBANNED!*", parse_mode="Markdown")
    except: pass
...
async def unmute_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    t = update.message.reply_to_message.from_user if update.message.reply_to_message else None
    if not t: await update.message.reply_text("🔊 _Reply karke /unmute_", parse_mode="Markdown"); return
    try:
